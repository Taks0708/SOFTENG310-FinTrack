...
describe("transactionService", () => {
    ...
    describe("getUserTransactionsByPage", () => {
        ...
        it("should return paginated transactions for a user", async () => {
            ...
            expect(poolQueryStub.firstCall.args[0].text).to.include(
                "SELECT * FROM transactions"
            );
            expect(poolQueryStub.firstCall.args[0].values).to.deep.equal([
                userID,
                10,
                10,
        ...
        it("should handle errors and throw the error", async () => {
            ...
            try {
                ...
                await transactionService.getUserTransactionsByPage(
                    userID,
                    pageNumber
                );
                ...
                expect(error.message).to.equal(
                    "error when getting transactions"
                ); // Expect the error message to be 'error when getting transactions'
                expect(poolQueryStub.calledOnce).to.be.true; // Expect the pool query function to be called once
            }
        });
    });
    ...
    describe("makeTransaction", () => {
        ...
        it("should successfully make a transaction and update balance", async () => {
            ...
            ); // Expect the first query to be an insert query
            expect(poolQueryStub.firstCall.args[0].values).to.deep.equal([
                userID,
                amount,
                title,
            ...
            expect(poolQueryStub.secondCall.args[0].text).to.include(
                "UPDATE users SET balance"
            ); // Expect the second query to be an update query
            expect(poolQueryStub.secondCall.args[0].values).to.deep.equal([
                amount,
                userID,
            ]);
        });
    ...
    describe("deleteTransaction", () => {
        ...
        it("should successfully delete a transaction and update balance", async () => {
            ...
            }); // Expect the result to be successful with a success message
        ...
        it("should handle errors when transaction does not exist", async () => {
            ...
            }); // Expect the result to be unsuccessful with an error message
            expect(poolQueryStub.calledOnce).to.be.true; // Expect the pool query function to be called once
        });
    ...
    describe("editTransaction", () => {
        ...
        it("should handle errors and throw the error", async () => {
            ...
            poolQueryStub.rejects(
                new Error("Error updating balance or editting the transaction")
            );
            ...
            try {
                ...
                expect(error.message).to.equal(
                    "Error updating balance or editting the transaction"
                ); // Expect the error message to be 'error deleting transaction'
                expect(poolQueryStub.calledOnce).to.be.true; // Expect the pool query function to be called once
            }
        });
    });
    ...
    describe("getTransaction", () => {
        ...
        it("should handle errors and throw the error", async () => {
            // Stub the pool query function to reject with an error
            poolQueryStub.rejects(
                new Error("error when getting a transaction")
            );
            ...
            try {
                ...
                expect(error.message).to.equal(
                    "error when getting a transaction"
                ); // Expect the error message to be 'error when getting transaction'
                expect(poolQueryStub.calledOnce).to.be.true; // Expect the pool query function to be called once
            }
        });
    });
